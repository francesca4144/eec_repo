...
int main(int argc, char* argv[]) {
    ...
    if(jobNumber != -1) outName = TString::Format("/data/rke_group/francesca/ep/%s/ep_EECs_%dk_pTHat%d_%d.root",outDir.c_str(),nEvents/1000,pTHat,jobNumber);
    ...
    // need different process cause this is saying q/q interaction pythia.readString("HardQCD:all = on");
    //pythia.readString(TString::Format("PhaseSpace:pTHatMin = %d",pTHat).Data());
    //might break cause it could be python stuff
    pythia.readString("Beams:frameType = 2");
    pythia.readString("WeakBosonExchange:ff2ff(t:gmZ) = on");
    pythia.settings.parm("PhaseSpace:Q2Min", pTHat);
